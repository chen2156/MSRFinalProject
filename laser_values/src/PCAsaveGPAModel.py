...
if __name__ == "__main__":
    ...
    #Set to right directory
    finalComponents, randIndex, totalNumImages = PCAImagesCompression("/home/chen2156/laserData/src/laser_values/src/newTrainingImages/unWarpedImages/", numImages)
    ...
    numDatapoints = len(ObservedData)
    #x = np.atleast_2d(np.linspace(0, 360, 100)).T
    kernel = RBF()
    y = np.array([])
    #approximate image with right LIDAR distance
    for ind in range(len(randIndex)):
      yInd = round(numDatapoints / totalNumImages * randIndex[ind])
